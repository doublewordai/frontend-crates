...
def r_reason(fam, text):
    if fam == "hunyuan":
        return hunyuan_markup(f"<think>{text}</think>")
    if fam == "kimi_k3":
...
_QWEN3_MARKUP = re.compile(
    r"</?think>|</?tool_call>|<function=([^>]*)>|</function>|<parameter=([^>]*)>|</parameter>"
)


def hunyuan_markup(text):
    """Spell Qwen3 markup in `text` as Hunyuan's `:opensource`-suffixed markup."""
    def spell(match):
        token = match[0]
        if match[1] is not None:
            return f"<tool_call:opensource>{match[1]}<tool_sep:opensource>"
        if match[2] is not None:
            return f"<arg_key:opensource>{match[2]}</arg_key:opensource><arg_value:opensource>"
        return {
            "<think>": "<think:opensource>",
            "</think>": "</think:opensource>",
            "<tool_call>": "<tool_calls:opensource>",
            "</tool_call>": "</tool_calls:opensource>",
            "</function>": "</tool_call:opensource>",
            "</parameter>": "</arg_value:opensource>",
        }[token]
    return _QWEN3_MARKUP.sub(spell, text)


def qwen3_as_hunyuan(input_text):
    """Translate a Qwen3 input into Hunyuan without changing its meaning.

    Qwen3 trims parameter values, so its inputs wrap each value in newlines;
    Hunyuan keeps every byte of a value, so those go before the markup is
    respelled.
    """
    text = re.sub(r"(<parameter=[^>\n]*>)\n", r"\1", input_text)
    return hunyuan_markup(text.replace("\n</parameter>", "</parameter>"))
...
def _entry(spec, fam):
    ...
    if isinstance(spec, dict) and set(spec) <= set(FAMILIES) and "verdict" not in spec:
        if fam == "deepseek_v4" and fam not in spec:
            return spec["qwen3"]
        if fam in DERIVED_INPUTS and fam not in spec:
            return spec[DERIVED_INPUTS[fam][0]]
        return spec[fam]
    return spec
...
def _build_edge_cases(fam, specs):
    ...
    for edge_case in specs:
        ...
        if fam not in per_fam:
            ...
            elif fam in DERIVED_INPUTS and DERIVED_INPUTS[fam][0] in per_fam:
                source, translate, translate_fill = DERIVED_INPUTS[fam]
                source_input, vllm_spec, dynamo_spec, *rest = per_fam[source]
                rest = [translate_fill(fill) if isinstance(fill, str) else fill for fill in rest]
                per_fam = dict(per_fam)
                per_fam[fam] = (translate(source_input), vllm_spec, dynamo_spec, *rest)
            else:
                raise KeyError(
